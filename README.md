...
## 📂 DDD 폴더 및 파일 구조 (예시: Event Server)

각 서버(Auth, Event)는 도메인 중심으로 폴더 구조를 구성하여 관심사를 분리합니다.
...
- **Mongoose 스키마:** `infrastructure/persistence/mongodb/schemas/` 에 위치.
...
1.  프로젝트 루트 디렉토리에서 다음 명령어를 실행하여 모든 서비스를 빌드하고 실행합니다.
    ...
    ```
2.  각 서비스는 `.env` 파일에 정의된 포트(예: Gateway: 3000, Auth: 3001, Event: 3002)로 실행됩니다.
3.  API 문서는 각 서비스의 `/api-docs` 경로에서 확인할 수 있습니다 (예: `http://localhost:3001/api-docs` - Auth Server).
